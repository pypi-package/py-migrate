...
def create_db(
    history_table: str = typer.Option(
        None,
        "--history-table",
        help="Override history table name (Fallback: PEEWEE_MIGRATION_TABLE env)",
    ),
    db_url: str = typer.Option(
        None, "--database", help="Override database URL (Fallback: DATABASE_URL env)"
    ),
) -> None:
    """Create the database."""
...
@app.command()
def run(
    version: str = typer.Argument(..., help="Version of the migration"),
    ...
    _set_history_table(history_table)
    run_command(
        version=version, direction=direction, fake=fake, db_url=_get_db_url(db_url)
    )
...
@app.command()
def up(
    version: str = typer.Argument(None, help="Specific version to run (optional)"),
...
def help(ctx: typer.Context) -> None:
    ...
    typer.echo(ctx.parent.get_help())  # type: ignore
...
def main(
    ...
    env: str = typer.Option(
        None, "--env", "-e", help="Set the environment (Fallback: PW_MIGRATE_ENV env)"
    ),
) -> None:
    ...
    for d in ["migrations", "seeds"]:
        if os.path.exists(d) and not os.path.isdir(d):
            Console().print(
                f"[bold red]Error:[/bold red] '{d}' exists in your project but is a file, not a directory!"
            )
            Console().print(
                f"[yellow]Please delete or rename the '{d}' file so the framework can use it as a directory.[/yellow]"
            )
