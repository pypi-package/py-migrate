...
class MigratorRunner:
    def __init__(self, db: Database, migrate_dir: str = "migrations"):
        ...
        self.router = Router(
            ...
            migrate_table="_pw_migrate_peewee_engine",
        )
    ...
    def _get_compiled_migrator(self):
        ...
        applied = (
            ...
            .order_by(MigrationHistory.version.asc())
        )
        for m in applied:
    ...
    def _do_run_up(self, steps: int = None, fake: bool = False) -> list:
            ...
            if os.path.exists(filepath):
                ...
                if m.checksum and m.checksum != current_checksum:
                    raise ChecksumMismatch(
                        f"Migration {m.version}_{m.name}.py has been modified since it was applied!"
                    )
        ...
        for filename in pending:
            ...
            if not fake:
                ...
                self.router.run_one(
                    migration_name, migrator, fake=False, downgrade=False
                )
            ...
            m = MigrationHistory.get_or_none(version=version)  # type: ignore
            if m:
                m.status = "UP"
    ...
    def _do_rollback(self, to_rollback: list, fake: bool) -> list:
        ...
        for m in to_rollback:
            ...
            if not fake:
                migrator = self._get_compiled_migrator()
                self.router.run_one(
                    migration_name, migrator, fake=False, downgrade=True
                )
    ...
    def run_single(
        self, version: str, direction: str = "up", fake: bool = False
    ) -> str:
    ...
    def _do_run_single(
        ...
        if direction == "up":
            if (
                MigrationHistory.select()
                .where(
                    MigrationHistory.version == version, MigrationHistory.status == "UP"
                )
                .exists()
            ):
