...
class Seeder:
    ...
    def discover_seeds(self) -> list:
        if not os.path.exists(self.seeds_dir):
            return []
        files = [f for f in os.listdir(self.seeds_dir) if f.endswith('.py') and not f.startswith('__')]
        return sorted(files)
    ...
    def run_all(self) -> list:
        ...
        for seed_file in seeds:
            ...
            if not hasattr(module, "seed"):
                raise MigrationError(f"Seed file {seed_file} must have a 'seed(database)' function.")
